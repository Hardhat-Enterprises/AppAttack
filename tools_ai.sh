...
generate_ai_insights() {
    ...
    if [[ "$ai_insights" == "y" ]]; then
        # Using API key from appattacktool@gmail.com (password: AppAttack2024!)
        API_KEY="AIzaSyAEnNWC__vKAX0nKHHbhKqjHxhN4TGEGgk"
        ...
        case $choice in
            1)
                ...
                if [[ "$output_to_file" == "y" ]]; then
                        ...
                        osv_output=$(osv-scanner --format table -r "$directory")  
                ...
                generate_ai_insights "$osv_output" "$output_to_file" "$output"
                echo -e "${GREEN}Operation completed.${NC}"
                ;;
            2)
                ...
                case $snyk_option in
                    1)   read -p "Enter directory to scan (current directory ./): " directory
                         if [[ "$output_to_file" == "y" ]]; then
                        ...
                        ;;
                    2)  read -p "Enter directory to scan (current directory ./): " directory
                        if [[ "$output_to_file" == "y" ]]; then
                ...
                generate_ai_insights "$snyk_output" "$output_to_file" "$output"
                echo -e "${GREEN}Operation completed.${NC}"
                ;;
            3)
                ...
                generate_ai_insights "$brakeman_output" "$output_to_file" "$output"
                echo -e "${GREEN}Operation completed.${NC}"
                ;;
            4)
                ...
                generate_ai_insights "$NMAP_OUTPUT" "$output_to_file" "$output"
                echo -e "${GREEN}Operation completed.${NC}"
                ;;
            5)
                ...
                generate_ai_insights "$nikto_output" "$output_to_file" "$output"
                echo -e "${GREEN}Operation completed.${NC}"
                ;;
            6)
                # Launch Legion
